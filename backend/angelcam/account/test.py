...
class ExternalTokenAuthenticationTest(TestCase):
    ...
    def test_successful_authentication(self, mock_get):
        ...
        request = factory.get('/some-url', HTTP_AUTHORIZATION='PersonalAccessToken validtoken')
    ...
    @patch('requests.get')
    def test_no_token_header(self, mock_get):
        factory = RequestFactory()
    ...
    @patch('requests.get')
    def test_invalid_token_header(self, mock_get):
        factory = RequestFactory()
        request = factory.get('/some-url', HTTP_AUTHORIZATION='InvalidHeader')
        auth = ExternalTokenAuthentication()
        with self.assertRaises(AuthenticationFailed):
            auth.authenticate(request)

    @patch('requests.get')
    def test_invalid_token_type(self, mock_get):
        factory = RequestFactory()
        request = factory.get('/some-url', HTTP_AUTHORIZATION='Bearer validtoken')
        auth = ExternalTokenAuthentication()
        self.assertIsNone(auth.authenticate(request)) 
    ...
    def test_invalid_authentication(self, mock_get):
        ...
        mock_get.return_value.json.return_value = {'email': 'test@example.com'}
        factory = RequestFactory()
        request = factory.get('/some-url', HTTP_AUTHORIZATION='PersonalAccessToken invalidtoken')
        auth = ExternalTokenAuthentication()
